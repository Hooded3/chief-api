...
app.get('/api/stats', (req, res) => {
    ...
    pool.query(
        ...
        (err, results) => {
            ...
            console.log(`Stats loaded in ${Date.now() - start}ms`);
            quickJson(res, results[0] || { residents: 0, families: 0, family_heads: 0, pending: 0 });
        }
    );
});

// Quick residents - limited fields
app.get('/api/residents', (req, res) => {
    ...
    pool.query(
        ...
        (err, results) => {
            ...
            console.log(`Residents loaded in ${Date.now() - start}ms`);
            quickJson(res, results);
        }
    );
});
...
app.post('/api/login', (req, res) => {
    ...
    let query = userType === 'staff' 
        ? 'SELECT id, username, role, full_name, is_master_admin, village_id FROM admin_users WHERE username = ?' 
        : 'SELECT id, username, role FROM resident_portal_access WHERE username = ?';
    
    pool.query(query, [username], (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
        if (results.length === 0) return res.status(401).json({ error: 'Invalid credentials' });
        const user = results[0];
        // For now, accept password as-is (you should use bcrypt)
        res.json({
            success: true,
            user: {
                id: user.id,
